...
## Dos versiones: usa la que te convenga

- **`La-Sala-Chester.html`** → un único archivo con todo dentro (estilos,
  JavaScript y fotos incrustadas). Ábrelo con doble clic y funciona
  siempre, sin depender de ningún otro archivo. Es la opción a prueba
  de descargas sueltas.
- **`index.html` + `style.css` + `script.js` + `assets/`** → la misma
  web troceada en archivos independientes. Se abre igual, pero además
  es la que te conviene si vas a tocar el diseño o el texto más
  adelante (más fácil de editar que un archivo único de 1,4&nbsp;MB).

Si solo descargas `index.html` suelto, sin `style.css` ni `script.js`
al lado, el navegador lo abre sin ningún estilo (así es como se ve HTML
sin su hoja de estilos). No es un fallo de diseño: es que le faltan las
piezas de al lado. Para evitarte ese lío, usa `La-Sala-Chester.html`.

## Cómo verla ahora mismo

Abre `La-Sala-Chester.html` (o `index.html`, si tienes toda la carpeta)
con doble clic, o desde un editor con "Live Server".
